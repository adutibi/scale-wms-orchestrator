...
- Auth: `orchestrator/src/auth.js` validates Bearer JWT (exp only; `AUTH_APP_ID` env just toggles auth on/off; unset = disabled; `/health` always open; signature NOT verified yet). `Authorization` header is stripped before forwarding to RabbitMQ
